...
task=mask_${task1}_${task2}_${task3}
...
python ./fairseq_mask/fairseq/fairseq_cli/train.py $TASK1_path \
...
python ./fairseq_mask/fairseq/fairseq_cli/train.py $TASK2_path \
...
if [ -f ./checkpoint/${s}${t}/${task2}/checkpoint_*_40000.pt ]; then
  ...
  cp ./checkpoint/${s}${t}/${task3}/${task2}_20000.pt ./checkpoint/${s}${t}/${task}/checkpoint_last.pt
fi

#forward distillation: train 4-7w steps
python ./fairseq_mask/fairseq/fairseq_cli/train.py $TASK3_path \
   ...
   --save-interval-updates 2000 \
   --keep-last-epochs 0 \
   --fp16-scale-tolerance 0.1 > ./checkpoint/${s}${t}/${task3}/train_${s}${t}_${task}_sub_${task3}.log 2>&1
wait
